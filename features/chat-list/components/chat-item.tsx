  ...
  return (
    <div
      className={`
        ...
        ${isSelected ? "bg-wa-panel border-r-4 border-wa-green" : "hover:bg-wa-hover"}
      `}
      onClick={onClick}
    >
      <div className="relative flex-shrink-0">
        <Avatar className="w-12 h-12">
          <AvatarImage src={chat.contact.avatar || "/placeholder.svg?height=48&width=48"} className="object-cover" />
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <h3 className="font-medium text-wa-primary truncate text-[15px]">{chat.contact.name}</h3>
            {chat.isPinned && <Pin className="w-3 h-3 text-wa-muted flex-shrink-0 rotate-45" />}
          </div>
          <div className="flex items-center gap-1 flex-shrink-0 ml-2">
            <span className="text-xs text-wa-muted">{formatTime(chat.lastMessage.timestamp)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-2">
            ...
            {chat.contact.isGroup && <Volume2 className="w-3 h-3 text-wa-muted" />}
          </div>
        </div>
      </div>
    </div>
